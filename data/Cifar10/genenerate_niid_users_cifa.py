...
# Replace power-law-based sampling with Dirichlet distribution using np.random.dirichlet
alpha = np.ones((10 * NUM_USERS, NUM_LABELS))  # Flatten alpha to 1D
props = np.random.dirichlet(alpha.flatten(), size=1).reshape((10, NUM_USERS, NUM_LABELS))
...
# Assign remaining samples using Dirichlet distribution
for user in trange(NUM_USERS):
    for j in range(NUM_LABELS):
        ...
        if idx[l] + num_samples <= len(cifa_data[l]):
            ...
            idx[l] += num_samples
            print("check len of user:", user, j,
